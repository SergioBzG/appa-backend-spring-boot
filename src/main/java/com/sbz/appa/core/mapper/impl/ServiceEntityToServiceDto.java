...
public class ServiceEntityToServiceDto implements Mapper<ServiceEntity, ServiceDto> {
    ...
    public ServiceDto mapTo(ServiceEntity serviceEntity) {
        return ServiceDto.builder()
                ...
                .type(serviceEntity.getType().toString())
                ...
                .originNation(serviceEntity.getOriginNation().toString())
                .destinationNation(serviceEntity.getDestinationNation().toString())
                .originCheckpoint(serviceEntity.getOriginCheckpoint().toString())
                .destinationCheckpoint(serviceEntity.getDestinationCheckpoint().toString())
                .carriageEntity(getCarriageDto(serviceEntity))
                .packageEntity(getPackageDto(serviceEntity))
                .guide(
                        GuideDto.builder()
                                .id(serviceEntity.getGuide().getId())
                                .currentNation(serviceEntity.getGuide().getCurrentNation().toString())
                                .currentCheckpoint(serviceEntity.getGuide().getCurrentCheckpoint().toString())
                                .build()
                )
                .build();
    }
